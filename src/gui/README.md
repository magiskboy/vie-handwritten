...
1. **Load Model** — pick a `*.weights.h5` checkpoint. Config/charset are resolved automatically (`config_used.yaml` beside the checkpoint, else `configs/default.yaml`). Decode defaults to `beam_lm` when `lm/vi.binary` exists; otherwise falls back to `greedy`.
